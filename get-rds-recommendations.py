...
def get_rds_recommendations(db_instance, region, account_id):
    ...
    try:
        ...
        finding_reason_codes_str = 'not available'
        ...
        if response.get('rdsDBRecommendations'):
            ...
            finding_reason_codes = recommendation.get('instanceFindingReasonCodes', [])
            finding_reason_codes_str = ','.join(finding_reason_codes) if finding_reason_codes else 'not available'
        ...
        recommendations = [
            ...
            finding, finding_reason_codes_str,
            futureInstanceClass, futureCpuUtilization, futureMemoryUtilization, futureDatabaseConnections,
            instance_recommendation_engine
        ]
        ...
        if 'OptInRequired' in str(e):
            ...
            return [db_instance['DBInstanceIdentifier'], db_instance['Engine'], db_instance['DBInstanceClass'],
                    'Compute Optimizer not enabled', 'Compute Optimizer not enabled', 'Compute Optimizer not enabled',
                    'Compute Optimizer not enabled', 'not available',
                    'not available', 'not available', 'not available', 'not available', db_instance['Engine']]
        else:
...
def create_csv_file(data):
    headers = [
        ...
        'Finding', 'Finding_Reason_Codes',
        'Recommended_Instance_Class', 'Recommended_vCPU', 'Recommended_Memory', 'Recommended_Max_CPU_Util', 'Recommended_Max_Memory_Util', 'Recommended_Max_DB_Connections',
        ...
        try:
            ...
            if response['DBInstances']:
                for db_instance in response['DBInstances']:
                    ...
                    (db_instance_id_rec, engine_rec, current_class, current_cpu_util, current_memory_util, current_db_connections,
                        finding, finding_reason_codes_str, future_class, future_cpu_util, future_memory_util, future_db_connections, recommended_engine) = recommendations
                    ...
                    listArrRec = [
                        ...
                        finding, finding_reason_codes_str,
                        future_class, vcpuNew, memNew, future_cpu_util, future_memory_util, future_db_connections,
